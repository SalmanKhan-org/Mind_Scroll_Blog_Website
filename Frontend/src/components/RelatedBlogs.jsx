  ...
  return (
    <div className='p-2'>
      {blogData?.blogs && blogData?.blogs?.length > 0 && (<h2 className='text-lg font-semibold '>Related Blogs</h2>)}
          ...
          blogData?.blogs?.map((blog, index) => blog._id !== blogId ? <Link to={`https://mindscroll-i13q.onrender.com/blog/${blog?.category?.name}/${blog?.slug}`} key={index + blog.__id} className='flex items-center gap-2 '>
