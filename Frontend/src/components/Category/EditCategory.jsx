...
const EditCategory = () => {
    ...
    const { id } = useParams();
    const { data: category, loading, error } = useFetch(`${API_END_POINT}/category/${id}`, {
                method: 'get',
                credentials: 'include'
    }, []);
    ...
    const onSubmit = async (values) => {
        try {
            const response = await axios.put(`${API_END_POINT}/category/${id}`, values, {
                headers: {
    ...
    useEffect(() => {
        if (category?.category) {
            form.setValue('name', category?.category?.name);
            form.setValue('slug', category?.category?.slug);
        }
    }, [category?.category]);
